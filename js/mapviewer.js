...
function init() {
    ...
    const urlParameters = visioweb.Utils.getURLParameters(undefined, false);
    ...
    let addPlaces = places => {
        ...
        _.each(places, place => {
            ...
            addListItem(placeItem, placeListElement, () => {
                essential.venue.goToPlace({id: place.id, radius: essential._mapviewer.camera.position.radius, heading: essential._mapviewer.camera.heading})
                    .then(() => {
...
function handleInitialParameters(initialParameters) {
    ...
    if (initialParameters.place) {
        essential.venue.goToPlace({id: initialParameters.place});
    }
